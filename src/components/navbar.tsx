    ...
    <>
      <header className="sticky top-0 z-40 w-full bg-background/90 backdrop-blur-md border-b border-[rgba(255,255,255,0.05)] px-4 md:px-8 py-4 flex justify-between items-center">
        <Link href="/" className="flex items-center gap-2 group">
          <div className="w-8 h-8 bg-primary flex items-center justify-center shadow-[0_0_10px_rgba(74,222,128,0.3)] group-hover:brightness-110 transition-all">
        ...
        <div className="flex items-center gap-4">
            ...
            {user ? (
               <div className="flex items-center gap-4">
                 <Link href="/profile" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                    ...
                    <img src={user.user_metadata.avatar_url || 'https://picsum.photos/seed/picsum/200/200'} className="w-8 h-8 bg-surface-container border border-[rgba(255,255,255,0.1)]" alt="Avatar" />
                 </Link>
                 <button onClick={handleLogout} className="text-on-surface hover:text-error transition-colors" title="Logout">
          ...
          <button onClick={() => setIsOpen(true)} className="block md:hidden p-2 text-on-surface hover:text-white transition-colors" aria-label="Open Menu">
      ...
      {/* Mobile Backdrop */}
      <div
        className={`fixed inset-0 bg-[#121212]/60 backdrop-blur-sm z-40 transition-opacity duration-300 md:hidden ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        onClick={() => setIsOpen(false)}
      ></div>

      {/* Mobile Half-Sidebar */}
      <aside
        className={`fixed top-0 right-0 bottom-0 w-[65%] sm:w-[50%] bg-[#121212] border-l border-[rgba(255,255,255,0.05)] shadow-2xl z-50 transform transition-transform duration-300 ease-out flex flex-col md:hidden ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="p-4 border-b border-[rgba(255,255,255,0.05)] flex justify-between items-center bg-[#1a120c]">
          <span className="font-bold text-on-surface-variant text-xs tracking-wider uppercase">Menu</span>
          <button onClick={() => setIsOpen(false)} className="p-1.5 text-on-surface hover:text-white transition-colors" aria-label="Close Menu">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <nav className="p-4 flex-1 space-y-2 overflow-y-auto">
          ...
          {menus.map((item) => {
            ...
            if (item.external) {
               return (
                 <a key={item.name} href={item.href} target="_blank" rel="noopener noreferrer" className={linkClasses} onClick={() => setIsOpen(false)}>
            ...
            return (
              <Link key={item.name} href={item.href} onClick={() => setIsOpen(false)} className={linkClasses}>
        ...
        <div className="p-4 border-t border-[rgba(255,255,255,0.05)] bg-[#1a120c]">
           {user ? (
            <div className="flex flex-col gap-4">
              <Link href="/profile" onClick={() => setIsOpen(false)} className="flex items-center gap-3 p-2 active:bg-surface-container-high transition-colors group">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={user.user_metadata.avatar_url || 'https://picsum.photos/seed/picsum/200/200'} alt="Avatar" className="w-10 h-10 bg-surface-container border border-[rgba(255,255,255,0.1)]" />
                  <div className="flex flex-col overflow-hidden">
                    <span className="text-sm font-bold text-white group-hover:text-primary truncate uppercase tracking-wider">{user.user_metadata.full_name || user.email}</span>
                    <span className="text-xs text-on-surface-variant uppercase tracking-wider truncate">{user.user_metadata.user_name || 'Developer'}</span>
                  </div>
              </Link>
              <button
